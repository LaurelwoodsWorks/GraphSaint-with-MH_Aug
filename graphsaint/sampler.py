...
class SAINTSampler:
    ...
    def __init__(self, node_budget, dn, g, train_nid, num_workers_sampler, num_subg_sampler=10000,
        ...
        graph_fn = graph_fn.replace(".npy", "")    ###################
        norm_fn = norm_fn.replace(".npy", "")    ###################
        ...
        if os.path.exists(graph_fn):
            ...
            for i in glob("./" + graph_fn + "/*"):
                self.subgraphs.append(np.load(i))
            # self.subgraphs = np.array(self.subgraphs) # ValueError: Inhomogeneous array

            aggr_norm = np.load(norm_fn + '/aggr_norm.npy', allow_pickle=True)
            loss_norm = np.load(norm_fn + '/loss_norm.npy', allow_pickle=True)
            ...
            os.makedirs('./{}'.format(graph_fn), exist_ok=True) ############################
            os.makedirs('./{}'.format(norm_fn), exist_ok=True) ############################
            ...
            # np.save(graph_fn, self.subgraphs)       ############### Problem : Inhomogenous numpy array can't be saved
            ############# Split and save numpy arrays individually
            for i, subgraph in enumerate(self.subgraphs):
                np.save(graph_fn + "/" + "{:03d}".format(i) + ".npy", subgraph)
            ...
            np.save(norm_fn + "/" + "aggr_norm" + ".npy", aggr_norm)
            np.save(norm_fn + "/" + "loss_norm" + ".npy", loss_norm)
